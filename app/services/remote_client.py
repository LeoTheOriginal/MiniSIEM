...
class RemoteClient:
    ...
    def run(self, command, sudo_password=None):
        ...
        Args:
            ...
            sudo_password: jeśli podane, komenda zostanie zaopatrzona w hasło przez bash
        """
        if not self.client:
            raise ConnectionError("Brak połączenia SSH")

        # Jeśli komenda wymaga sudo I mamy hasło, opakuj w bash -c z printf
        ...
        stdin, stdout, stderr = self.client.exec_command(actual_command)

        out = stdout.read().decode().strip()
        err = stderr.read().decode().strip()

        print(f"📊 [RemoteClient.run] STDOUT length: {len(out)}, STDERR length: {len(err)}")
